...
/obj/effect/landmark/cycling_trader/proc/trader_arrive()
	...
	GLOB.global_announcer.autosay("Trader has arrived at the [clan_name].", "Doisac Flight Control", announce_channel, LANGUAGE_SANGHEILI)

/obj/effect/landmark/cycling_trader/proc/trader_leave()
	has_trader = 0
	time_cycle_trader = world.time + rand(trader_gap_min, trader_gap_max)
	GLOB.global_announcer.autosay("Trader has departed the [clan_name].", "Doisac Flight Control", announce_channel, LANGUAGE_SANGHEILI)
